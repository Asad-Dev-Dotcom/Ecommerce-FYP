...
import { useGetDashboardStatsQuery } from '../../../redux/apis/dashboardApis';

const LowStockAlert = () => {
  const { data: dashboardStats } = useGetDashboardStatsQuery();
  const lowStockProducts = dashboardStats?.data?.lowStockAlerts || [];
        ...
        {lowStockProducts.map(product => (
          <div key={product.id} className="flex items-center p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            ...
            <div className="flex-1">
              <h4 className="text-base font-semibold text-gray-800 m-0 mb-1">{product.name}</h4>
